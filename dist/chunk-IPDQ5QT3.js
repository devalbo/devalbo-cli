...
var createMetrics = getCreateFunction((store) => {
  ...
  const [
    ...
    store,
    getUndefined,
    (value) => isNaN(value) || isUndefined(value) || isTrue(value) || isFalse(value) || value === EMPTY_STRING ? void 0 : value * 1,
    addListener,
    callListeners
  );
...
var BrowserStoreFSDriver = class {
  ...
  async init() {
    ...
    this.initPromise = (async () => {
      try {
        const { createOpfsPersister } = await import("./persister-browser-EXYVAJRC.js");
        ...
        try {
          const { createIndexedDbPersister } = await import("./persister-indexed-db-DJN4YUFZ.js");
          ...
          const { createLocalPersister } = await import("./persister-browser-EXYVAJRC.js");
...
var toFilePath = (path) => unsafeAsFilePath(normalizeBrowserPath(path));
var toDirectoryPath = (path) => unsafeAsDirectoryPath(normalizeBrowserPath(path));

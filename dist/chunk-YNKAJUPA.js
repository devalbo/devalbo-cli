...
var getWatcher = async () => createWatcherService();
...
var stringifyBft = (node) => JSON.stringify(node, null, 2);
var sortedEntries = (entries) => Object.entries(entries).sort(([left], [right]) => left.localeCompare(right));
...
var pathOps = () => isNode() ? path : path.posix;
var getDefaultCwd = () => {
  if (!isNode()) return "/";
...
var splitFsPath = (input) => input.split("/").filter(Boolean);
...
var writeTextFile = async (cwd, requested, content) => {
  const targetPath = resolveFsPath(cwd, validatePathArg(requested, "Invalid file path"));
  const driver = await getDriver();
  await driver.writeFile(toFilePath(targetPath), new TextEncoder().encode(content));
  return targetPath;
};
var writeBytesFile = async (cwd, requested, data) => {
  const targetPath = resolveFsPath(cwd, validatePathArg(requested, "Invalid file path"));
  const driver = await getDriver();
...
var encodeDirectoryAbsolute = async (dirPath) => {
  const driver = await getDriver();
  const entries = sortEntries(await driver.readdir(toDirectoryPath(dirPath)));
...
var decodeDirectoryAbsolute = async (node, destPath) => {
  const driver = await getDriver();
  await driver.mkdir(toDirectoryPath(destPath));
...
var exportDirectoryAsBft = async (cwd, sourcePath = ".", outputPath) => {
...
var importBftToLocation = async (cwd, bftFilePath, locationName) => {
  ...
  const text2 = new TextDecoder().decode(await driver.readFile(toFilePath(absoluteBftPath)));
  const result = await importBftTextToLocation(cwd, text2, locationName);
  return {
    bftFilePath: absoluteBftPath,
    targetPath: result.targetPath
  };
};
var importBftTextToLocation = async (cwd, bftText, locationName) => {
...
${parsed.error}`);
    ...
    try {
      const cwd = options?.cwd ?? getDefaultCwd();
      const { path: path2, entry } = await statPath(cwd, requested);
...
var Spinner = ({ type = "dots", label = "Loading..." }) => {
  ...
  return /* @__PURE__ */ jsx2(Text2, { children: `${label} ${frames[index]}` });
};
...
var BrowserShellProvider = ({ children }) => {
  ...
  return /* @__PURE__ */ jsx3(ShellContext.Provider, { value: value2, children });
};
...
var TerminalShellProvider = ({ children }) => {
  ...
  return /* @__PURE__ */ jsx4(ShellContext.Provider, { value: value2, children });
};
...
var resolveMimeTypeHandler = (mimeType) => {
...
var markDefaultMimeHandlersRegistered = () => {
  defaultsRegistered = true;
};
var areDefaultMimeHandlersRegistered = () => defaultsRegistered;
...
// packages/ui/src/file-handlers/markdown-utils.ts
var escapeHtml = (input) => input.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#39;");
var sanitizeUrl = (raw) => {
  const value2 = raw.trim();
  if (value2.startsWith("http://") || value2.startsWith("https://") || value2.startsWith("mailto:") || value2.startsWith("#") || value2.startsWith("/")) {
    return value2;
  }
  return "#";
};
var renderInline = (input) => {
  let out = escapeHtml(input);
  out = out.replace(/`([^`]+)`/g, "<code>$1</code>");
  out = out.replace(/\*\*([^*]+)\*\*/g, "<strong>$1</strong>");
  out = out.replace(/\*([^*]+)\*/g, "<em>$1</em>");
  out = out.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (_m, text2, href) => {
    const safeHref = sanitizeUrl(href);
    return `<a href="${safeHref}" target="_blank" rel="noreferrer">${text2}</a>`;
  });
  return out;
};
var toMarkdownText = (content) => typeof content === "string" ? content : new TextDecoder().decode(content);
var renderMarkdownHtml = (markdown) => {
...
`);
    ...
    if (heading) {
      ...
      const value2 = heading[2] ?? "";
      const level = marks.length;
      html.push(`<h${level}>${renderInline(value2)}</h${level}>`);
      continue;
    }
...
var MarkdownEdit = ({ content, onSave, onChange }) => {
  const incoming = useMemo3(() => toMarkdownText(content), [content]);
  const [baseline, setBaseline] = useState4(incoming);
  const [draft, setDraft] = useState4(incoming);
  ...
  return /* @__PURE__ */ jsxs("div", { children: [
    ...
    /* @__PURE__ */ jsxs("div", { style: { marginTop: "8px", display: "flex", gap: "8px", alignItems: "center" }, children: [
      /* @__PURE__ */ jsx5(
        "button",
        {
          onClick: () => {
          ...
          children: isSaving ? "Saving..." : "Save Markdown"
        }
      ),
      /* @__PURE__ */ jsx5("span", { style: { color: "#94a3b8" }, children: dirty ? "Unsaved changes" : "Saved" })
    ] })
  ] });
};
...
var MarkdownView = ({ content }) => {
  const markdown = useMemo4(() => toMarkdownText(content), [content]);
  const html = useMemo4(() => renderMarkdownHtml(markdown), [markdown]);
  return /* @__PURE__ */ jsx6("div", { style: previewStyle, children: /* @__PURE__ */ jsx6("div", { dangerouslySetInnerHTML: { __html: html } }) });
};
...
var MarkdownViewEdit = ({ content, onSave, onChange }) => {
  const incoming = useMemo5(() => toMarkdownText(content), [content]);
  const [baseline, setBaseline] = useState5(incoming);
  const [draft, setDraft] = useState5(incoming);
  const [isSaving, setIsSaving] = useState5(false);
  const dirty = draft !== baseline;
  const previewHtml = useMemo5(() => renderMarkdownHtml(draft), [draft]);
  useEffect3(() => {
    if (!dirty) {
      setBaseline(incoming);
      setDraft(incoming);
  ...
  return /* @__PURE__ */ jsxs2("div", { children: [
    /* @__PURE__ */ jsxs2("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }, children: [
      /* @__PURE__ */ jsx7(
        "textarea",
        {
          value: draft,
      ...
      /* @__PURE__ */ jsx7("div", { style: { background: "#020617", border: "1px solid #334155", borderRadius: "8px", padding: "12px", color: "#e2e8f0", lineHeight: 1.6 }, children: /* @__PURE__ */ jsx7("div", { dangerouslySetInnerHTML: { __html: previewHtml } }) })
    ] }),
    /* @__PURE__ */ jsxs2("div", { style: { marginTop: "8px", display: "flex", gap: "8px", alignItems: "center" }, children: [
      /* @__PURE__ */ jsx7(
        "button",
        {
          onClick: () => {
      ...
      /* @__PURE__ */ jsx7("span", { style: { color: "#94a3b8" }, children: dirty ? "Unsaved changes" : "Saved" })
    ] })
  ] });
};
...
var coerceBytes = (content) => typeof content === "string" ? new TextEncoder().encode(content) : content;
var ImageFilePreview = ({ content, mimeType, path: path2 }) => {
  const bytes = useMemo6(() => coerceBytes(content), [content]);
  const [url2, setUrl] = useState6("");
  useEffect4(() => {
  ...
  if (!url2) return /* @__PURE__ */ jsx8("div", { style: { color: "#94a3b8" }, children: "Loading preview..." });
  return /* @__PURE__ */ jsxs3("div", { style: { display: "flex", flexDirection: "column", gap: "8px" }, children: [
    /* @__PURE__ */ jsx8("img", { src: url2, alt: path2, style: { maxWidth: "100%", maxHeight: "560px", objectFit: "contain", borderRadius: "8px", border: "1px solid #334155" } }),
    /* @__PURE__ */ jsx8("div", { style: { color: "#94a3b8", fontSize: "12px" }, children: mimeType })
  ] });
};

// packages/ui/src/file-handlers/text-file-view-edit.tsx
import { useEffect as useEffect5, useMemo as useMemo7, useState as useState7 } from "react";
import { jsx as jsx9, jsxs as jsxs4 } from "react/jsx-runtime";
var coerceText = (content) => typeof content === "string" ? content : new TextDecoder().decode(content);
var TextFileViewEdit = ({ content, onSave, onChange }) => {
  const incoming = useMemo7(() => coerceText(content), [content]);
  const [baseline, setBaseline] = useState7(incoming);
  const [draft, setDraft] = useState7(incoming);
  const [isSaving, setIsSaving] = useState7(false);
  const [remoteChanged, setRemoteChanged] = useState7(false);
  const dirty = draft !== baseline;
  useEffect5(() => {
    if (!dirty) {
      setBaseline(incoming);
  ...
  return /* @__PURE__ */ jsxs4("div", { children: [
    ...
    /* @__PURE__ */ jsxs4("div", { style: { marginTop: "8px", display: "flex", gap: "8px", alignItems: "center", flexWrap: "wrap" }, children: [
      /* @__PURE__ */ jsx9(
        "button",
        {
          onClick: () => {
      ...
      /* @__PURE__ */ jsx9("span", { style: { color: "#94a3b8" }, children: dirty ? "Unsaved changes" : "Saved" }),
      remoteChanged && /* @__PURE__ */ jsx9("span", { style: { color: "#f59e0b" }, children: "Changed externally while editing" })
    ] })
  ] });
};

// packages/ui/src/file-handlers/defaults.ts
var registerDefaultMimeTypeHandlers = () => {
...
var TextInput = ({
  ...
  const [internalValue, setInternalValue] = useState8(value2 ?? defaultValue ?? "");
  ...
  return /* @__PURE__ */ jsx10(Text3, { dimColor, children: display });
};

// packages/ui/src/primitives/select.tsx
import React11 from "react";
import { Box as Box2, Text as Text4, useInput as useInput3 } from "ink";
...
import { createContext as createContext2, useContext as useContext2, useEffect as useEffect7, useRef } from "react";
import { jsx as jsx16 } from "react/jsx-runtime";
var ShellRuntimeContext = createContext2(null);
function useShellRuntime() {
  return useContext2(ShellRuntimeContext);
}
function ShellRuntimeProvider({
  ...
  return /* @__PURE__ */ jsx16(ShellRuntimeContext.Provider, { value: value2, children });
}
...
function ShellContent({
  ...
  const [connectivityFallback] = useState9(() => new BrowserConnectivityService());
  const connectivity = connectivityProp ?? connectivityFallback;
  const [input, setInput] = useState9("");
  const [inputKey, setInputKey] = useState9(0);
...
var InteractiveShell = ({
  ...
  if (runtime === "terminal") {
    return /* @__PURE__ */ jsx17(TerminalShellProvider, { children: /* @__PURE__ */ jsx17(ShellContent, { ...shellContentProps, runtime: "terminal" }) });
  }
  return /* @__PURE__ */ jsx17(BrowserShellProvider, { children: /* @__PURE__ */ jsx17(ShellContent, { ...shellContentProps, runtime: "browser" }) });
};
...
async function startInteractiveCli(opts) {
  ...
  const App = () => {
    const [cwd, setCwd] = useState10(initialCwd);
...
async function createApp(options) {
  ...
  function App({
    ...
    return /* @__PURE__ */ jsx19(StoreContext.Provider, { value: store, children: /* @__PURE__ */ jsx19(AppConfigProvider, { config, children: /* @__PURE__ */ jsx19(ShellRuntimeProvider, { value: runtimeValue, bindToCli: true, children: children ?? /* @__PURE__ */ jsx19(InteractiveShell, { welcomeMessage }) }) }) });
  }
...
import { useState as useState12, useEffect as useEffect8, useRef as useRef2 } from "react";
function useValidParse(source, parse2) {
  const [validDoc, setValidDoc] = useState12(null);
  const [parseError, setParseError] = useState12(null);

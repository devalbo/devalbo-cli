...
export const getContact = (store: Store, id: ContactId): ContactRow | null => {
...
export const setContact = (store: Store, id: ContactId, contact: ContactRowInput): void => {
  const parsed = ContactRowSchema.parse(contact);
  store.setRow(CONTACTS_TABLE, id, parsed as Row);
};

export const listContacts = (store: Store): Array<{ id: ContactId; row: ContactRow }> => {
  ...
  return Object.entries(table).flatMap(([id, row]) => {
    ...
    return parsed ? [{ id: id as ContactId, row: parsed }] : [];
  });
};

export const deleteContact = (store: Store, id: ContactId): void => {
...
export const searchContacts = (store: Store, query: string): Array<{ id: ContactId; row: ContactRow }> => {
...
export const linkContactToPersona = (store: Store, contactId: ContactId, personaId: PersonaId): void => {
